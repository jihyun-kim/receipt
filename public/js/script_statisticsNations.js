...
function ajaxGetStatistics(year, month){
	$.ajax({
		type : "GET",
		url : "/statistics/api/search2/" + year + "&" + month,  
			success: function(results){
				...
				results.forEach( function ( result, index, array) {
					...
					$('#table').append("<tr'>" + "<td align='center'>" + (index + 1)  + "</td>"  + "<td>" + result.nationnm  + "</td>"  + StrTd +  "</tr>"); 
				});
				var StrTd = setTd(TotalCnts);
				//console.log('---Total StrTd---', StrTd);
				$('#table').append("<tr style='font-weight:bold'>" + "<td>" + ' ' + "</td>" + "<td>" + '합   계' + "</td>" + StrTd +  "</tr>"); 
			},
			error : function(e) {
				console.log("ERROR: ", e);
			}
